...
rf1=train(x=x_train,y=y_train,
          ...
          tuneLength = 30)
names(getModelInfo())
data1<-read.csv("Elasso.csv",header = TRUE)
...
x_test<-data1[,1:10]
y_test<-data1[,11]
data.predict1=predict(rf1,newdata=x_test,type='prob')
data.predict<-ifelse(data.predict1[,2]>0.625,1,0)
confusionMatrix(factor(data.predict,levels=c(0,1),labels=c("NO",'YES')),y_test,positive = 'YES')
ci_sen<-binconf(24,26)
ci_sen
ci_spe<-binconf(64,74)
ci_spe
